...
class pedido{
    ...
    public function registrarDetalle($_params){
        $sql = "INSERT INTO `detalle_pedidos`(`pedidos_id`, `articulos_id`, `precio`, `cantidad`) 
        VALUES (:pedidos_id,:articulos_id,:precio,:cantidad)";

        $resultado = $this->cn->prepare($sql);

        $_array = array(
            ":pedidos_id" =>$_params['pedidos_id'],
            ":articulos_id" =>$_params['articulos_id'],
    ...
    public function mostrarDetallePorIdPedido($id){
        $sql = "SELECT dp.id, ar.titulo, dp.precio, dp.cantidad, ar.foto 
                ...
                INNER JOIN articulos ar ON ar.id= dp.articulos_id
